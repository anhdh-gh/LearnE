...
public class LogsActivityAOPHandler {
    ...
    public Object logsActivityAnnotation(ProceedingJoinPoint point,
        ...
        if(objectResponse instanceof BaseResponse){
              if(((BaseResponse<?>) objectResponse).getMeta() != null){
                mapCustomizeLog.put("error_code", ((BaseResponse<?>) objectResponse).getMeta().getCode());
                logger.info(objectMapper.writeValueAsString(objectResponse), entries(mapCustomizeLog));
            }
        }
